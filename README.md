...
- Searchable directory with 61 native connection types, including OpenAI, Anthropic, DeepSeek, Groq, NVIDIA NIM, Azure, and custom endpoints. Unsupported desktop/specialist protocols are explicitly marked unavailable.
- Native account linking for OpenRouter (PKCE), Kimi Code, Kilo Code, GitHub Copilot, Codex, and Grok Build (device authorization), with encrypted token storage and serialized renewal. No external 9router service is required.
- Per-connection ranked model preferences with full-catalog search, free-model filtering, and optional restrictions on automatic routing.
- Provider-reported token/request limits and reset countdowns, OpenRouter/DeepSeek credit checks, Codex/Copilot/Kimi account quotas, and per-connection gateway usage. Unknown quotas remain unknown. See [provider setup and limitations](docs/provider-hub.md).
